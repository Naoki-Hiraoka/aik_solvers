...
namespace prioritized_acc_inverse_kinematics_solver_sample{
  void sample16_4limb_region(){
    ...
    }
    {
      ...
      std::shared_ptr<aik_constraint::RegionConstraint> constraint = std::make_shared<aik_constraint::RegionConstraint>();
      constraint->A_link() = robot->link("LARM_WRIST_R");
      constraint->A_localpos() = cnoid::Vector3(0.0,0.0,-0.02);
      constraint->B_link() = nullptr;
      constraint->B_localpos() = cnoid::Vector3(0.0,0.7,0.0);
      constraint->C().resize(1,3);
      constraint->C().insert(0,1) = 1.0;
