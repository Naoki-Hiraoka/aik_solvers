...
namespace aik_constraint {
  ...
  void calcCMJacobianShape(const std::vector<cnoid::LinkPtr>& joints,
    ...
    for(size_t i=0;i<joints.size();i++){
      ...
      cols += IKConstraint::getJointDOF(joints[i]);
    }
    for(size_t i=0;i<forces.size();i++){
      cols += forces[i]->DOF();
    }
      ...
      for(int i=0;i<2;i++){
        ...
        if(jacobianColMap.find(robot->rootLink()) != jacobianColMap.end()){
          if(robot->rootLink()->isFreeJoint()){
            ...
            for(size_t row=0;row<3;row++){
              for(size_t j=0;j<IKConstraint::getJointDOF(robot->rootLink());j++){
        ...
        for(size_t j=0;j<robot->numJoints();j++){
          if(jacobianColMap.find(robot->joint(j)) != jacobianColMap.end()){
            ...
            for(size_t row=0;row<3;row++){
              for(size_t k=0;k<IKConstraint::getJointDOF(robot->joint(j));k++){
      ...
      for(size_t i=0;i<2; i++){
        ...
        if(jacobianColMap.find(robot->rootLink()) != jacobianColMap.end()){
          if(robot->rootLink()->isFreeJoint()){
            ...
            for(size_t j=0;j<3;j++){
              for(size_t k=0;k<IKConstraint::getJointDOF(robot->rootLink());k++){
        ...
        for(size_t j=0;j<robot->numJoints();j++){
          if(jacobianColMap.find(robot->joint(j)) != jacobianColMap.end()){
            ...
            for(size_t k=0;k<3;k++){
              for(size_t d=0;d<IKConstraint::getJointDOF(robot->joint(j));d++){
      ...
      for(int i=0;i<2;i++){
        ...
        if(jacobianColMap.find(robot->rootLink()) != jacobianColMap.end()){
          if(robot->rootLink()->isFreeJoint()){
            int idx = jacobianColMap[robot->rootLink()];
            for(size_t row=0;row<3;row++){
              for(size_t j=3;j<IKConstraint::getJointDOF(robot->rootLink());j++){//並進は無視
                tripletList.push_back(Eigen::Triplet<double>(row,idx+j,1));
              }
            }
          }
      ...
      for(size_t i=0;i<2; i++){
        ...
        if(jacobianColMap.find(robot->rootLink()) != jacobianColMap.end()){
          if(robot->rootLink()->isFreeJoint()){
            int col_idx = jacobianColMap[robot->rootLink()];
            for(size_t j=0;j<3;j++){
              for(size_t k=3;k<IKConstraint::getJointDOF(robot->rootLink());k++){ // 並進は無視
