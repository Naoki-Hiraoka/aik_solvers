...
namespace aik_constraint{
  ...
  public:
    ...
    // 必ず,状態更新(全リンクのF_extは0. 探索変数のリンクのddqは0でも良いし非0でもよい) -> ForwardKinematics(true,true) -> calcCenterOfMass() -> calcInverseDynamics()してrootLinkが受ける力(rootLinkまわり)をrootLink->F_ext()に入れる -> update() -> getError() / getJacobian / getMin/MaxIneq / getJacobianIneq / getDrawOnObjects の順で呼ぶので、同じ処理を何度も行うのではなく最初に呼ばれる関数で1回だけ行って以降はキャッシュを使ってよい
